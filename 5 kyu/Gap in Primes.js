...
//
// gap(6,100,110) --> nil or {0, 0} : between 100 and 110 we have 101, 103, 107, 109 but 101-107is not a 6-gap because there is
// 103in between and 103-109is not a 6-gap because there is 107in between.

//My solution
function gap(g, m, n) { //No native methods challenge
    let array = ['', ''], isPrime = false;
    for (let i = m; i <= n; i++) {
        isPrime = true;
        ...
        if (isPrime) [array[0], array[1]] = [array[1], i];
        if (array[1] - array[0] == g &&
            array[0] != '' && array[1] != '') return array;
    }
    return null;
}