...
//1) Can you write a solution that will return null for both [] and [ x ] though? ( This is not tested, but you can write your own 
//example test. )
...
function firstNonConsecutive(arr) {
    for (let i = 0; i < arr.length - 1; ++i) { //same number of runs
      if (arr[i] + 1 !== arr[i + 1]) {
        return arr[i + 1]
      }
    }
